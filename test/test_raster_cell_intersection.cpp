...
void
check_cell_intersections(Raster<float>& rci, const std::vector<std::vector<float>>& v)
{
...
TEST_CASE("Geometry extent larger than raster", "[raster-cell-intersection]")
{
    ...
    check_cell_intersections(lr, { { 1.00, 0.50 }, { 1.00, 0.50 }, { 0.50, 0.25 } });

    Raster<float> ur = raster_cell_intersection(g2, context, g.get());

    check_cell_intersections(ur, { { 0.50, 0.25 }, { 1.00, 0.50 }, { 1.00, 0.50 } });
...
TEST_CASE("Bouncing off boundary", "[raster-cell-intersection]")
{
    ...
    check_cell_intersections(rci, { { 0.25, 0.25 }, { 0.4375, 0.3125 } });
}
...
TEST_CASE("Starts on horizontal boundary, moving right", "[raster-cell-intersection]")
{
    ...
    check_cell_intersections(rci, { { 0.25, 0.5, 0.25 }, { 0.50, 1.0, 0.50 }, { 0.50, 1.0, 0.50 } });
}
...
TEST_CASE("Result indexing is correct", "[raster-cell-intersection]")
{
    ...
    Matrix<float> expected{ { { 0.25, 0.50, 0.50, 0.50, 0.50, 0.25 },
