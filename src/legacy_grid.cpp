...
namespace exactextract {
    ...
    LegacyGrid::LegacyGrid(double xmin, double ymin, double xmax, double ymax, double dx, double dy) :
            xmin{xmin},
            ymin{ymin},
            xmax{xmax},
            ymax{ymax},
        ...
        // of m_dx, we can find the number of cells using round instead of floor.
    ...
    size_t LegacyGrid::get_column(double x) const {
    ...
    size_t LegacyGrid::get_row(double y) const {
    ...
    LegacyGrid LegacyGrid::shrink_to_fit(const Box &b) const {
        return shrink_to_fit(b.xmin, b.ymin, b.xmax, b.ymax);
    }

    LegacyGrid LegacyGrid::shrink_to_fit(double x0, double y0, double x1, double y1) const {
    ...
    std::unique_ptr<Cell> LegacyGrid::get_cell_ptr(size_t row, size_t col) const {
    ...
    bool LegacyGrid::compatible_with(const exactextract::LegacyGrid &b) const {
    ...
    LegacyGrid LegacyGrid::common_grid(const LegacyGrid &b) const {
    ...
    bool LegacyGrid::operator==(const LegacyGrid &b) const {
        return
            xmin == b.xmin &&
            ymin == b.ymin &&
            xmax == b.xmax &&
